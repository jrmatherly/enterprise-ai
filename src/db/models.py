...
class Tenant(Base):
    ...
    # Note: knowledge_bases relationship removed - KnowledgeBase uses flexible tenant_id (String)
...
class User(Base):
    ...
    # Note: Sessions and Documents use flexible user_id (String) to support both UUID and better-auth IDs
...
class KnowledgeBase(Base):
    ...
    tenant_id and owner_id are String types to support both UUID (dev) and better-auth IDs (SSO).
    """
    ...
    tenant_id: Mapped[str] = mapped_column(String(255), nullable=False)  # Flexible: UUID or string
    ...
    # Owner (for personal KBs) - flexible ID (UUID or better-auth ID)
...
class Document(Base):
    ...
    )  # Flexible: UUID or better-auth ID
    ...
    # Note: uploaded_by relationship removed - uses flexible uploaded_by_id (String)
