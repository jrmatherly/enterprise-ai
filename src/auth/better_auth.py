...
async def validate_session_token(token: str) -> tuple[BetterAuthSession, BetterAuthUser] | None:
    ...
    # better-auth tokens may include a signature after a '.' separator
    # The database stores only the token part, not the signature
    if "." in token:
        token = token.split(".")[0]
